...
新版本已适配不同的应用场景，增加用户自定义标签内容功能。通过本地json文件管理标签。第一次使用将自动创建标签文件。
...
跨平台兼容： 基于 Python 和 customtkinter 构建，完美支持 Windows系统, macOS 和 Linux 系统将在未来适配。

#### ⚙️实用工具箱

内置图片压缩： 在重命名的同时，提供可选的图片压缩功能，可自定义质量和最大尺寸，优化存储空间。

图片高斯模糊：在使用场景中，如果图片涉及到电话号码地址等用户敏感信息，现在不需要再额外打开其他软件去模糊处理。直接进去模糊编辑功能即可为图片区域添加模糊处理。
